...
/**
  ...
  * @param  Char 要显示的丢?个字符，范围：ASCII可见字符
  * @retval ??
  */
...
void OLED_Showfloat(uint8_t Line, uint8_t Column, float fNumber, uint8_t Length, uint8_t fLength)
{
    if (fNumber < 0)
    {
    ...
    OLED_ShowNum(Line, Column + 1, (uint32_t)fNumber, Length);
    OLED_ShowChar(Line, Column + Length + 1, '.');
    OLED_ShowNum(Line, Column + Length + 2, (uint32_t)(fNumber * OLED_Pow(10, fLength)) % OLED_Pow(10, fLength), fLength);
}
...
void OLED_Init(void)
{
    ...
    OLED_WriteCommand(0x40);	//设置显示弢?始行
